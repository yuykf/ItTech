...
public class RoundImageView extends AppCompatImageView {
    ...
    private Paint mSelectPaint; //左下角文字
    private Paint mCirclePaint; //左下角圆圈
    /**
     * 圆角的半径
     */
    ...
    public RoundImageView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        type = a.getInt(R.styleable.RoundImageView_flynn_type, TYPE_OVAL);

        mBorderColor = a.getColor(R.styleable.RoundImageView_flynn_border_color, Color.WHITE);
        mBorderWidth = a.getDimension(R.styleable.RoundImageView_flynn_border_width, 0);
        mCornerRadius = a.getDimension(R.styleable.RoundImageView_flynn_corner_radius, dp2px(10));
        mLeftTopCornerRadius = a.getDimension(R.styleable.RoundImageView_flynn_leftTop_corner_radius, 0);
        mLeftBottomCornerRadius = a.getDimension(R.styleable.RoundImageView_flynn_leftBottom_corner_radius, 0);
        mRightTopCornerRadius = a.getDimension(R.styleable.RoundImageView_flynn_rightTop_corner_radius, 0);
        mRightBottomCornerRadius = a.getDimension(R.styleable.RoundImageView_flynn_rightBottom_corner_radius, 0);
        textWord = a.getString(R.styleable.RoundImageView_flynn_select_word);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.labelImageView);
        try {
            labelSrc = typedArray.getResourceId(R.styleable.labelImageView_label_src, R.drawable.icon_gdt_channel_logo);
            showLabel = typedArray.getBoolean(R.styleable.labelImageView_show_label, false);
        } catch (Exception ex) {
    ...
    protected void init() {
        ...
        mSelectPaint.setColor(Color.parseColor("#4AA9F4"));
        mSelectPaint.setTextAlign(Paint.Align.RIGHT);
        mSelectPaint.setTextSize(40f);
    ...
    protected boolean hasThisDraw = true;
    ...
    protected void onDraw(Canvas canvas) {
        if (hasThisDraw) {
            ...
            if (type == TYPE_ROUND) {
                ...
                canvas.drawCircle(mRoundRect.left + 30, mRoundRect.bottom - 25, 22, mCirclePaint);
                canvas.drawText(textWord, mRoundRect.left+42, mRoundRect.bottom-13, mSelectPaint);
            ...
            if (showLabel) {
                //画画操作
                Bitmap bitmap = BitmapFactory.decodeResource(getResources(), labelSrc);
     ...
     * 设置标签资源
     */
    public void setLabelSrc(int labelSrc) {
